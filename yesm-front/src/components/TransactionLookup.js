...
const TransactionLookup = () => {
  ...
  const { walletTransactions, fetchWalletTransactions, isLoading, error } = useWalletData(API_URL);
  const { tokenPrices } = useTokenPrices(API_URL, walletTransactions);
      ...
      {walletTransactions.length > 0 ? (
        <div className="grid gap-6 lg:grid-cols-2 bg-gradient-to-b from-[#FFB6C1] to-[#FFE4B5] p-8 rounded-lg shadow-lg">
          {walletTransactions.map((transaction) => {
            ...
            const soldKey = `eth:${soldAddress.toLowerCase()}`;
            const boughtKey = `eth:${boughtAddress.toLowerCase()}`;
            console.log(`Looking up prices for keys: Sold=${soldKey}, Bought=${boughtKey}`);

            // Previous approach for fetching prices with additional fallback checks
            const currentSoldPrice = tokenPrices && tokenPrices[soldKey] ? tokenPrices[soldKey].usd || 0 : 0;
            const currentBoughtPrice = tokenPrices && tokenPrices[boughtKey] ? tokenPrices[boughtKey].usd || 0 : 0;
